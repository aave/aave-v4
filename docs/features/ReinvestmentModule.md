...
The Hub imposes no on-chain cap on the amount the controller may sweep in a single call. Cap enforcement is the controller contract's responsibility. The controller may not reclaim more than the current `asset.swept` amount; specifying a larger amount causes an arithmetic underflow and reverts.
...
Interest accrued by external strategies on swept funds is not tracked on-chain by the Hub. The Hub records no interest accrual against `asset.swept`. Per the interface specification, all interest earned on swept liquidity is distributed off-chain.

When the controller returns a yield surplus to the Hub by transferring more than the swept principal, the surplus sits in the Hub's ERC-20 balance without an immediate accounting entry in `asset.liquidity`. This surplus cannot be booked via `reclaim` once outstanding swept principal is fully reclaimed, because `reclaim` is bounded by `asset.swept`. This design separates Hub principal accounting from external yield flows; the controller is responsible for the full yield distribution lifecycle.
