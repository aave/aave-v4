...
contract BorrowIndex_Scenario3Test is LiquidityHubScenarioBaseTest {
  ...
  function precondition(Stages stage) internal override {
    ...
    if (stage == Stages.t0) {
      ...
      spoke4Amounts.draw.t3 = 1e18;
    } else if (stage == Stages.t4) {
      spoke4Amounts.supply.t4 = 1e8;
    }
  }
  ...
  function initialAssertions(Stages stage) internal override {
    ...
    if (stage == Stages.t0) {
      ...
      assertEq(spokes_i.spoke4.t3.baseDebt, 0, 't3_i Spoke4 base debt');
      assertEq(
  ...
  function exec(Stages stage) internal override {
    ...
    if (stage == Stages.t1) {
      ...
      Utils.supply({
        ...
        assetId: assetId,
        spoke: address(spoke4),
        amount: spoke4Amounts.supply.t4,
        riskPremiumRad: 0,
        user: bob,
        to: address(spoke4)
      });
  ...
  function finalAssertions(Stages stage) internal override {
    if (stage == Stages.t0) {
      ...
      assertEq(spokes_f.spoke4.t2.baseDebt, 0, 't2_f Spoke4 base debt');
      assertEq(spokes_f.spoke4.t2.lastUpdateTimestamp, 0, 't2_f Spoke4 lastUpdateTimestamp');
    } else if (stage == Stages.t3) {
      ...
      assertEq(spokes_f.spoke4.t3.baseDebt, spoke4Amounts.draw.t3, 't3_f Spoke4 base debt');
      assertEq(
      ...
      // asset
      assertEq(
        assets_f.assetData0.t4.baseBorrowIndex,
        assets_f.assetData0.t3.baseBorrowIndex.rayMul(states.cumulatedBaseInterest.t4),
        't4_f Asset index'
      );
      assertEq(
        assets_f.assetData0.t4.baseDebt,
        assets_f.assetData0.t3.baseDebt.rayMul(states.cumulatedBaseInterest.t4),
        't4_f Asset base debt'
      );
      assertEq(
        assets_f.assetData0.t4.lastUpdateTimestamp,
        timeAt(Stages.t4),
      ...
      // no action, should be the same as t1
      assertEq(
        spokes_f.spoke1.t4.baseBorrowIndex,
        spokes_f.spoke1.t1.baseBorrowIndex,
        't4_f Spoke1 index'
      );
      assertEq(spokes_f.spoke1.t4.baseDebt, spokes_f.spoke1.t1.baseDebt, 't4_f Spoke1 base debt');
      assertEq(
        spokes_f.spoke1.t4.lastUpdateTimestamp,
        spokes_f.spoke1.t1.lastUpdateTimestamp,
      ...
      // spoke4
      assertEq(
        spokes_f.spoke4.t4.baseBorrowIndex,
        assets_f.assetData0.t4.baseBorrowIndex,
        't4_f Spoke4 index'
      );
      assertEq(
        spokes_f.spoke4.t4.baseDebt,
        spokes_f.spoke4.t3.baseDebt.rayMul(states.cumulatedBaseInterest.t4),
        't4_f Spoke4 base debt'
      );
      assertEq(
        spokes_f.spoke4.t4.lastUpdateTimestamp,
        timeAt(Stages.t4),
