...
contract BorrowIndex_Scenario2Test is LiquidityHubScenarioBaseTest {
  ...
  function precondition(Stages stage) internal override {
    ...
    if (stage == Stages.t1) {
      ...
      spoke4Amounts.draw.t[2] = 1e18;
    } else if (stage == Stages.t3) {
      spoke4Amounts.supply.t[3] = 1e8;
    }
  }
  ...
  function initialAssertions(Stages stage) internal override {
    ...
    if (stage == Stages.t0) {
      ...
      assertEq(spokes_i.spoke4.t[3].baseDebt, spoke4Amounts.draw.t[2], 't3_i Spoke4 base debt');
      assertEq(
  ...
  function exec(Stages stage) internal override {
    ...
    if (stage == Stages.t1) {
      ...
      Utils.supply({
        ...
        assetId: assetId,
        spoke: address(spoke4),
        amount: spoke4Amounts.supply.t[3],
        riskPremiumRad: 0,
        user: bob,
        to: address(spoke4)
      });
  ...
  function finalAssertions(Stages stage) internal override {
    if (stage == Stages.t0) {
      ...
      assertEq(
        assets_f.assetData[0].t[2].baseBorrowIndex,
        assets_f.assetData[0].t[1].baseBorrowIndex.rayMul(states.cumulatedBaseInterest.t[2]),
        't2_f Asset index'
      );
      assertEq(
        assets_f.assetData[0].t[2].baseDebt,
        assets_f.assetData[0].t[1].baseDebt.rayMul(states.cumulatedBaseInterest.t[2]) +
          spoke4Amounts.draw.t[2],
      ...
      assertEq(spokes_f.spoke4.t[1].baseDebt, spoke4Amounts.draw.t[1], 't2_f Spoke4 base debt');
      assertEq(
      ...
      assertEq(
        spokes_f.spoke4.t[3].baseDebt,
        spokes_f.spoke4.t[2].baseDebt.rayMul(states.cumulatedBaseInterest.t[3]),
