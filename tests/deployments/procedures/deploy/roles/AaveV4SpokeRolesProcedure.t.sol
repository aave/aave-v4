...
contract AaveV4SpokeRolesProcedureTest is ProceduresBase {
  ...
  function test_getSpokeConfiguratorRoleSelectors() public view {
    ...
    assertEq(selectors[6], ISpoke.updateGate.selector);
    assertEq(selectors[7], ISpoke.updateReservePriceSource.selector);
  }

  function test_getSpokePositionTransferAdminRoleSelectors() public view {
    bytes4[] memory selectors = aaveV4SpokeRolesProcedureWrapper
      .getSpokePositionTransferAdminRoleSelectors();
    assertEq(selectors.length, 1);
    assertEq(selectors[0], ISpoke.transferPosition.selector);
  }
