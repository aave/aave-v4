...
/// @dev Validates the Global Dollar TokenizationSpoke replacement on a devnet where the Security Council
/// activation batch (`output/global-dollar-tokenization-spokes-activation.json`) has been executed.
/// Skipped unless TENDERLY_DEVNET_RPC is set.
contract GlobalDollarTokenizationSpokesActivationTest is Test {
  ...
  address internal constant NEW_WA_GLOBAL_DOLLAR_USDC = 0xaed7c529bD2878170B61C758DfAa215AC7a4FD07;
  address internal constant NEW_WA_GLOBAL_DOLLAR_USDT = 0xa0e97e45C2f89003730E467Bd484fA3eEcE5B4Cf;
  address internal constant NEW_WA_GLOBAL_DOLLAR_PT_USDG =
    0x7Df10B4A01350D2A1d95cFbE7c9207d7210A2663;

  address internal constant OLD_WA_GLOBAL_DOLLAR_USDC = 0x4131E0B2E7AFeCEAf3d3b4225aA61a3B2B7535b8;
  address internal constant OLD_WA_GLOBAL_DOLLAR_USDT = 0x8Dabe53E8cB991c57f0307F6f419E6D469b0deAA;
  address internal constant OLD_WA_GLOBAL_DOLLAR_PT_USDG =
    0x27eF1140364948A0E30E248297FfDFE5a4091ec4;

  address internal constant OLD_WA_GLOBAL_DOLLAR_USDC_HOLDER =
  ...
  function test_newSpokes_proxyAdminOwnership() public onlyDevnet {
    address[3] memory spokes = [
      NEW_WA_GLOBAL_DOLLAR_USDC,
      NEW_WA_GLOBAL_DOLLAR_USDT,
      NEW_WA_GLOBAL_DOLLAR_PT_USDG
    ];
  ...
  function test_newSpokes_activationState() public onlyDevnet {
    _assertSpokeConfig({
      ...
      spoke: NEW_WA_GLOBAL_DOLLAR_USDC,
      underlying: USDC,
      expectedAddCap: ADD_CAP
    });
    _assertSpokeConfig({
      assetId: USDT_ASSET_ID,
      spoke: NEW_WA_GLOBAL_DOLLAR_USDT,
      underlying: USDT,
      expectedAddCap: ADD_CAP
    });
    _assertSpokeConfig({
      assetId: PT_USDG_ASSET_ID,
      spoke: NEW_WA_GLOBAL_DOLLAR_PT_USDG,
  ...
  function test_oldSpokes_remainFrozen() public onlyDevnet {
    _assertSpokeConfig({
      assetId: USDC_ASSET_ID,
      spoke: OLD_WA_GLOBAL_DOLLAR_USDC,
      underlying: USDC,
      expectedAddCap: 0
    });
    _assertSpokeConfig({
      assetId: USDT_ASSET_ID,
      spoke: OLD_WA_GLOBAL_DOLLAR_USDT,
      underlying: USDT,
      expectedAddCap: 0
    });
    _assertSpokeConfig({
      assetId: PT_USDG_ASSET_ID,
      spoke: OLD_WA_GLOBAL_DOLLAR_PT_USDG,
      underlying: PT_USDG_24SEP2026,
      expectedAddCap: 0
    });
  }
  ...
  function test_newUsdcSpoke_depositAndRedeem() public onlyDevnet {
    _depositAndRedeem(NEW_WA_GLOBAL_DOLLAR_USDC, USDC, 1000e6);
  }

  function test_newUsdtSpoke_depositAndRedeem() public onlyDevnet {
    _depositAndRedeem(NEW_WA_GLOBAL_DOLLAR_USDT, USDT, 1000e6);
  }
  ...
  function test_newUsdcSpoke_depositAboveCapReverts() public onlyDevnet {
    ...
    vm.startPrank(USER);
    IERC20(USDC).forceApprove(NEW_WA_GLOBAL_DOLLAR_USDC, amount);
    vm.expectRevert(abi.encodeWithSelector(IHub.AddCapExceeded.selector, ADD_CAP));
    ITokenizationSpoke(NEW_WA_GLOBAL_DOLLAR_USDC).deposit(amount, USER);
    vm.stopPrank();
  }
  ...
  function test_newPtSpoke_depositReverts_zeroCap() public onlyDevnet {
    ...
    vm.startPrank(USER);
    IERC20(PT_USDG_24SEP2026).forceApprove(NEW_WA_GLOBAL_DOLLAR_PT_USDG, 100e6);
    vm.expectRevert(abi.encodeWithSelector(IHub.AddCapExceeded.selector, 0));
    ITokenizationSpoke(NEW_WA_GLOBAL_DOLLAR_PT_USDG).deposit(100e6, USER);
    vm.stopPrank();
  }
  ...
  function test_oldSpokes_depositsBlocked() public onlyDevnet {
    ...
    IERC20(USDC).forceApprove(OLD_WA_GLOBAL_DOLLAR_USDC, 100e6);
    vm.expectRevert(abi.encodeWithSelector(IHub.AddCapExceeded.selector, 0));
    ITokenizationSpoke(OLD_WA_GLOBAL_DOLLAR_USDC).deposit(100e6, USER);

    IERC20(USDT).forceApprove(OLD_WA_GLOBAL_DOLLAR_USDT, 100e6);
    vm.expectRevert(abi.encodeWithSelector(IHub.AddCapExceeded.selector, 0));
    ITokenizationSpoke(OLD_WA_GLOBAL_DOLLAR_USDT).deposit(100e6, USER);

    vm.stopPrank();
  }

  function test_oldUsdcSpoke_withdrawalsOpen() public onlyDevnet {
    ITokenizationSpoke oldSpoke = ITokenizationSpoke(OLD_WA_GLOBAL_DOLLAR_USDC);
    uint256 shares = oldSpoke.balanceOf(OLD_WA_GLOBAL_DOLLAR_USDC_HOLDER);
    assertGt(shares, 0);

    uint256 balanceBefore = IERC20(USDC).balanceOf(OLD_WA_GLOBAL_DOLLAR_USDC_HOLDER);
    vm.prank(OLD_WA_GLOBAL_DOLLAR_USDC_HOLDER);
    uint256 assets = oldSpoke.redeem(
      shares,
      OLD_WA_GLOBAL_DOLLAR_USDC_HOLDER,
      OLD_WA_GLOBAL_DOLLAR_USDC_HOLDER
    );

    assertGt(assets, 0);
    assertEq(
      IERC20(USDC).balanceOf(OLD_WA_GLOBAL_DOLLAR_USDC_HOLDER),
      balanceBefore + assets,
      'holder should be able to fully exit the frozen spoke'
    );
    assertEq(oldSpoke.balanceOf(OLD_WA_GLOBAL_DOLLAR_USDC_HOLDER), 0);
  }
  ...
  function _assertSpokeConfig(
    ...
    IHub hub = IHub(GLOBAL_DOLLAR_HUB);
