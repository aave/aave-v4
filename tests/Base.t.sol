...
abstract contract Base is Test {
  ...
  function configureTokenList() internal {
    ...
    hub1.addSpoke(address(tokenList.weth), address(spoke1), spokeConfig);
    hub1.addSpoke(address(tokenList.wbtc), address(spoke1), spokeConfig);
    hub1.addSpoke(address(tokenList.dai), address(spoke1), spokeConfig);
    hub1.addSpoke(address(tokenList.usdx), address(spoke1), spokeConfig);
    hub1.addSpoke(address(tokenList.usdy), address(spoke1), spokeConfig);
    ...
    hub1.addSpoke(address(tokenList.wbtc), address(spoke2), spokeConfig);
    hub1.addSpoke(address(tokenList.weth), address(spoke2), spokeConfig);
    hub1.addSpoke(address(tokenList.dai), address(spoke2), spokeConfig);
    hub1.addSpoke(address(tokenList.usdx), address(spoke2), spokeConfig);
    hub1.addSpoke(address(tokenList.usdy), address(spoke2), spokeConfig);
    hub1.addSpoke(address(tokenList.usdz), address(spoke2), spokeConfig);
    ...
    hub1.addSpoke(address(tokenList.dai), address(spoke3), spokeConfig);
    hub1.addSpoke(address(tokenList.usdx), address(spoke3), spokeConfig);
    hub1.addSpoke(address(tokenList.weth), address(spoke3), spokeConfig);
    hub1.addSpoke(address(tokenList.wbtc), address(spoke3), spokeConfig);

    vm.stopPrank();
  }

  /* @dev Configures Hub 2 with the following assets:
   * 0: WETH
   * 1: USDX
   * 2: DAI
   * 3: WBTC
  ...
  /* @dev Configures Hub 3 with the following assets:
   * 0: DAI
   * 1: USDX
   * 2: WBTC
   * 3: WETH
  ...
  function updateAssetFeeReceiver(
    ...
    assertEq(hub.getAssetConfig(asset), config);
  }
  ...
  function updateLiquidityFee(IHub hub, address asset, uint256 liquidityFee) internal pausePrank {
    IHub.AssetConfig memory config = hub.getAssetConfig(asset);
  ...
  function _updateSpokePaused(
    ...
    IHub.SpokeConfig memory spokeConfig = hub.getSpokeConfig(asset, spoke);
    spokeConfig.paused = paused;
    vm.prank(HUB_ADMIN);
    hub.updateSpokeConfig(asset, spoke, spokeConfig);

    assertEq(hub.getSpokeConfig(asset, spoke), spokeConfig);
  }
  ...
  function getSpokeInfo(
    ...
    return hub1.getSpoke(asset, spoke);
  }

  function getAssetInfo(address asset) internal view returns (IHub.Asset memory) {
  ...
  function minimumAssetsPerAddedShare(IHub hub, address asset) internal view returns (uint256) {
    return hub.previewAddByShares(asset, 1);
  }

  /// @dev Helper function to calculate asset amount corresponding to single drawn share
  function minimumAssetsPerDrawnShare(IHub hub, address asset) internal view returns (uint256) {
    return hub.previewRestoreByShares(asset, 1);
  }
  ...
  function getAddExRate(address asset) internal view returns (uint256) {
    return hub1.previewRemoveByShares(asset, MAX_SUPPLY_AMOUNT);
  }

  function getDebtExRate(address asset) internal view returns (uint256) {
    return hub1.previewRestoreByShares(asset, MAX_SUPPLY_AMOUNT);
  }

  function getDeficit(IHub hub, address asset) internal view returns (uint256) {
    return hub.getAsset(asset).deficit;
  }

  function getAssetDrawnRate(IHub hub, address asset) internal view returns (uint256) {
    return hub.getAsset(asset).drawnRate;
  }
  ...
  function _getValue(
    ...
    return
      (amount * IPriceOracle(spoke.ORACLE()).getReservePrice(reserveId)).wadDivDown(
        10 ** TestnetERC20(_underlying(spoke, reserveId)).decimals()
      );
  }
  ...
  function _calculateExactRestoreAmount(
    ...
    drawnRestored = hub1.previewRestoreByShares(
      asset,
      hub1.previewRestoreByAssets(asset, drawnRestored)
    );
  ...
  function _checkSuppliedAmounts(
    ...
    uint256 expectedSuppliedShares = hub1.previewAddByAssets(asset, expectedSuppliedAmount);
    assertEq(
    ...
    assertEq(
      hub1.getAddedAssets(asset) - _calculateBurntInterest(hub1, asset),
      expectedSuppliedAmount,
      string(abi.encodePacked('asset supplied amount ', label))
    );
    assertEq(
      hub1.getSpokeAddedShares(asset, address(spoke)),
      expectedSuppliedShares,
      string(abi.encodePacked('spoke supplied shares ', label))
    );
    assertEq(
      hub1.getSpokeAddedAssets(asset, address(spoke)),
      expectedSuppliedAmount,
  ...
  function _assertSpokeDebt(
    ...
    assertApproxEqAbs(
      hub1.getSpokeTotalOwed(asset, address(spoke)),
      expectedDrawnDebt + expectedPremiumDebt,
      3,
  ...
  function _assertAssetDebt(
    ...
    address asset = spoke.getReserve(reserveId).underlying;
    (uint256 actualDrawnDebt, uint256 actualPremiumDebt) = hub1.getAssetOwed(asset);
    assertApproxEqAbs(
      actualDrawnDebt,
      expectedDrawnDebt,
      1,
  ...
  function _calcMinimumCollAmount(
    ...
    uint256 collAssetUnits = 10 ** hub1.getAsset(collData.underlying).decimals;

    ISpoke.Reserve memory debtData = spoke.getReserve(debtReserveId);
    uint256 debtAssetUnits = 10 ** hub1.getAsset(debtData.underlying).decimals;
  ...
  function getAssetDrawnDebt(address asset) internal view returns (uint256) {
  ...
  function _withdrawLiquidityFees(IHub hub, address asset, uint256 amount) internal {
    Utils.mintFeeShares(hub, asset, ADMIN);
    uint256 fees = hub.getSpokeAddedAssets(asset, address(treasurySpoke));
    ...
    treasurySpoke.withdraw(address(hub), asset, amount);
  }
  ...
  function _getAssetLiquidityFee(address asset) internal view returns (uint256) {
    return hub1.getAssetConfig(asset).liquidityFee;
  }

  function _getFeeReceiver(IHub hub, address asset) internal view returns (address) {
    return hub.getAssetConfig(asset).feeReceiver;
  }

  function _getFeeReceiver(ISpoke spoke, uint256 reserveId) internal view returns (address) {
    return _getFeeReceiver(_hub(spoke, reserveId), spoke.getReserve(reserveId).underlying);
  }
  ...
  function _approveAllUnderlying(ISpoke spoke, address owner, address spender) internal {
    for (uint256 reserveId; reserveId < spoke.getReserveCount(); ++reserveId) {
      TestnetERC20 underlying = TestnetERC20(_underlying(spoke, reserveId));
      vm.prank(owner);
  ...
  function _mockInterestRateBps(
    ...
    vm.mockCall(
      ...
      abi.encodeCall(
        ...
        (asset, liquidity, drawn, deficit, swept)
      ),
      abi.encode(interestRateBps.bpsToRay())
    );
  }
  ...
  function _mockInterestRateRay(
    ...
    _mockInterestRateRay(address(irStrategy), interestRateRay, asset, liquidity, drawn, 0, 0);
  }
  ...
  function assertBorrowRateSynced(
    ...
    IHub.Asset memory assetData = targetHub.getAsset(asset);
    (uint256 drawn, ) = hub1.getAssetOwed(asset);

    vm.assertEq(
      assetData.drawnRate,
      IBasicInterestRateStrategy(assetData.irStrategy).calculateInterestRate(
        asset,
        assetData.liquidity,
        drawn,
        assetData.deficit,
  ...
  function assertHubLiquidity(IHub targetHub, address asset, string memory label) internal view {
    IHub.Asset memory assetData = targetHub.getAsset(asset);
    uint256 currentHubBalance = IERC20(asset).balanceOf(address(targetHub));
    assertEq(
      targetHub.getAssetLiquidity(asset),
      currentHubBalance,
  ...
  function getAssetPosition(
    ...
    return
      AssetPosition({
        ...
        addedAmount: hub.getAddedAssets(asset) - _calculateBurntInterest(hub, asset),
  ...
  function getSpokePosition(
    ...
    address asset = spoke.getReserve(reserveId).underlying;
    IHub.SpokeData memory spokeData = hub1.getSpoke(asset, address(spoke));
    (uint256 drawn, uint256 premium) = hub1.getSpokeOwed(asset, address(spoke));
    return
      SpokePosition({
        reserveId: reserveId,
        asset: asset,
        addedShares: spokeData.addedShares,
        addedAmount: hub1.getSpokeAddedAssets(asset, address(spoke)),
  ...
  function _calcUnrealizedFees(IHub hub, address asset) internal view returns (uint256) {
    IHub.Asset memory assetData = hub.getAsset(asset);
    uint256 lastDrawnIndex = assetData.drawnIndex;
    uint256 drawnIndex = assetData.drawnIndex.rayMulUp(
      MathUtils.calculateLinearInterest(assetData.drawnRate, uint40(assetData.lastUpdateTimestamp))
    );
    uint256 liquidityGrowth = assetData.drawnShares.rayMulUp(drawnIndex) -
      assetData.drawnShares.rayMulUp(lastDrawnIndex) +
      assetData.premiumShares.rayMulUp(drawnIndex) -
      assetData.premiumShares.rayMulUp(lastDrawnIndex);

    return liquidityGrowth.percentMulDown(assetData.liquidityFee);
  }
  ...
  function _getExpectedFeeReceiverAddedAssets(
    ...
    uint256 expectedFees = hub.getAsset(asset).realizedFees + _calcUnrealizedFees(hub, asset);
    assertEq(expectedFees, hub.getAssetAccruedFees(asset), 'asset accrued fees');
    return hub.getSpokeAddedAssets(asset, hub.getAsset(asset).feeReceiver) + expectedFees;
  }

  function _getAddedAssetsWithFees(IHub hub, address asset) internal view returns (uint256) {
    return
      hub.getAddedAssets(asset) +
      hub.getAsset(asset).realizedFees +
      _calcUnrealizedFees(hub, asset);
  }
}
