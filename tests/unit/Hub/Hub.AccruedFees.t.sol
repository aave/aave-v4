...
contract HubAccruedFeesTest is HubBase {
  ...
  function test_unrealizedFees_fuzz_basicAccrual(
    ...
    //   uint256 expectedAccruedFees = _getFeeReceiverAddedAssets(hub1, daiAssetId);
  ...
  /// @dev Verifies fees >= protocol cut over multiple iterations with realized fees
  function test_unrealizedFees_accountingInvariant() public {
    ...
    //   // Trigger accrual to convert unrealized -> realized fees
    //   Utils.add({
    //     hub: hub1,
    //     assetId: daiAssetId,
    //     caller: address(spoke1),
    ...
    //     user: alice
    //   });
    //   assertGt(hub1.getAsset(daiAssetId).realizedFees, 0);
  ...
  function test_unrealizedFees_90_10_split() public {
    ...
    assertEq(accruedFees, totalDelta.percentMulDown(90_00));

    uint256 supplierYield = hub1.getAddedAssets(daiAssetId) - SUPPLY_AMOUNT;
    assertEq(supplierYield, totalDelta.percentMulDown(10_00));
  ...
  function test_unrealizedFees_repeatedCycles() public {
    ...
    //   lastSharePrice = currentSharePrice;
    // }
    // uint256 finalAccruedFees = _getFeeReceiverAddedAssets(hub1, daiAssetId);
    // assertEq(
  ...
  function test_fuzz_unrealizedFees_gradualSharePriceGrowth(
    ...
    for (uint256 i = 0; i < 12; ++i) {
      ...
      // Accrue occasionally to test share price growth while realizedFees are earning interest
      if (i % 4 == 3) {
        Utils.add({
          hub: hub1,
          assetId: daiAssetId,
