...
//     );
//     vm.startPrank(ADMIN);
//     minDecimalUnderlying = hub1.addAsset(
//       address(usda),
...
//       })
//     );
//     hub1.addSpoke(minDecimalUnderlying, address(spoke1), spokeConfig);
//     vm.stopPrank();
//   }
...
//     _addLiquidity(minDecimalUnderlying, 100e18);
//     _drawLiquidity(minDecimalUnderlying, 45e18, true);

//     uint256 totalAddedAssets = hub1.getAddedAssets(minDecimalUnderlying);
//     uint256 totalAddedShares = hub1.getAddedShares(minDecimalUnderlying);
...
//     uint256 spokeAddedShares = hub1.getSpokeAddedShares(minDecimalUnderlying, address(spoke1));
...
//     _updateAddCap(minDecimalUnderlying, address(spoke1), newAddCap);

//     Utils.add({
//       hub: hub1,
//       underlying: minDecimalUnderlying,
...
//     test_add_fuzz_single_asset(address(tokenList.dai), alice, 100e18);
//   }

//   /// @dev User makes a first add, shares and assets amounts are correct, no precision loss
//   function test_add_fuzz_single_asset(address underlying, address user, uint256 amount) public {
//     _assumeValidSupplier(user);

//     underlying = bound(underlying, 0, hub1.getAssetCount() - 3); // Exclude duplicated DAI and usdy
//     amount = bound(amount, 1, MAX_SUPPLY_AMOUNT);

//     IERC20 underlying = IERC20(hub1.getAsset(underlying).underlying);

//     (uint256 drawnBefore, ) = hub1.getAssetOwed(underlying);
//     uint256 liquidityBefore = hub1.getAssetLiquidity(underlying);
//     vm.expectCall(
//       address(irStrategy),
//       abi.encodeCall(
//         IBasicInterestRateStrategy.calculateInterestRate,
//         (underlying, liquidityBefore + amount, drawnBefore, 0, 0)
//       )
//     );
...
//     uint256 shares = hub1.previewAddByAssets(underlying, amount);
//     vm.expectEmit(address(hub1));
//     emit IHubBase.Add(underlying, address(spoke1), shares, amount);

//     uint256 addedShares = hub1.add(underlying, amount);
//     vm.stopPrank();

//     // hub
//     assertEq(addedShares, shares);
//     assertEq(hub1.getAddedAssets(underlying), amount, 'hub asset addedAmount after');
//     assertEq(hub1.getAddedShares(underlying), shares, 'hub asset addedShares after');
//     assertEq(
//       hub1.getSpokeAddedAssets(underlying, address(spoke1)),
//       amount,
...
//     );
//     assertEq(hub1.getAsset(underlying).lastUpdateTimestamp, vm.getBlockTimestamp());
//     assertEq(
//       hub1.getAsset(underlying).liquidity,
//       liquidityBefore + amount,
//       'hub available liquidity after'
//     );
//     (uint256 drawnAfter, ) = hub1.getAssetOwed(underlying);
//     assertEq(drawnAfter, drawnBefore, 'hub drawn debt after');
//     assertBorrowRateSynced(hub1, underlying, 'hub1.add');
//     assertHubLiquidity(hub1, underlying, 'hub1.add');
//     // token balance
...
//     uint256 amount2
//   ) public {
//     underlying = bound(underlying, 0, hub1.getAssetCount() - 4); // Exclude duplicated DAI and usdy
//     amount = bound(amount, 1, MAX_SUPPLY_AMOUNT);
//     amount2 = bound(amount2, 1, MAX_SUPPLY_AMOUNT);

//     address underlying2 = underlying + 1;

//     IERC20 underlying = IERC20(hub1.getAsset(underlying).underlying);
//     IERC20 underlying2 = IERC20(hub1.getAsset(underlying2).underlying);

//     vm.startPrank(address(spoke1));
//     underlying.transferFrom(alice, address(hub1), amount);

//     vm.expectEmit(address(hub1));
//     emit IHubBase.Add(underlying, address(spoke1), amount, amount);

//     hub1.add(underlying, amount);
//     vm.stopPrank();
...
//     vm.expectEmit(address(hub1));
//     emit IHubBase.Add(underlying2, address(spoke2), amount2, amount2);

//     hub1.add(underlying2, amount2);
//     vm.stopPrank();
...
//       hub1.previewAddByAssets(underlying, amount),
//       'asset addedShares after'
//     );
//     assertEq(hub1.getAddedAssets(underlying), amount, 'asset addedAmount after');
//     assertEq(hub1.getAssetLiquidity(underlying), amount, 'asset liquidity after');
//     assertEq(
//       hub1.getAsset(underlying).lastUpdateTimestamp,
//       timestamp,
...
//     // asset2
//     assertEq(
//       hub1.getAddedShares(underlying2),
//       hub1.previewAddByAssets(underlying2, amount2),
//       'asset2 addedShares after'
//     );
//     assertEq(hub1.getAssetLiquidity(underlying2), amount2, 'asset2 liquidity after');
//     assertEq(
//       hub1.getAsset(underlying2).lastUpdateTimestamp,
//       timestamp,
//       'asset2 lastUpdateTimestamp after'
//     );
//     assertEq(
//       hub1.getSpokeAddedShares(underlying2, address(spoke2)),
//       hub1.previewAddByAssets(underlying2, amount2),
//       'spoke2 addedShares after'
//     );
//     assertEq(
//       hub1.getSpokeAddedAssets(underlying2, address(spoke2)),
//       amount2,
...
//     assertHubLiquidity(hub1, underlying2, 'hub1.add');
//   }
...
//       skipTime: skipTime
//     });
//     vm.assume(hub1.previewAddByAssets(underlying, amount) < amount);

//     params.drawnAmount = amount;
//     params.assetAddedAmount = hub1.previewRemoveByShares(underlying, params.assetAddedShares);
//     params.availableLiq = amount - params.drawnAmount;
//     params.spoke2AddedShares = hub1.getSpokeAddedShares(underlying, address(spoke2));
//     params.spoke2AddedAmount = hub1.previewRemoveByShares(underlying, params.spoke2AddedShares);
...
//       addAmount = minimumAssetsPerAddedShare(hub1, underlying);
...
//       assertGt(drawn, 0);
//       (drawn, ) = hub1.getSpokeOwed(underlying, address(spoke1));
//       assertGt(drawn, 0);
...
//       params.spoke1AddedAmount = hub1.previewRemoveByShares(underlying, params.spoke1AddedShares);
//       params.bobBalance -= addAmount;

//       // hub
//       assertGe(hub1.getAddedAssets(underlying), params.assetAddedAmount, 'hub addedAmount after');
//       assertGe(hub1.getAddedShares(underlying), params.assetAddedShares, 'hub addedShares after');
//       assertEq(hub1.getAssetLiquidity(underlying), params.availableLiq, 'asset liquidity after');
//       assertEq(
//         hub1.getAsset(underlying).lastUpdateTimestamp,
//         vm.getBlockTimestamp(),
//         'asset lastUpdateTimestamp after'
...
//       assertEq(
//         hub1.getSpokeAddedAssets(underlying, address(spoke1)),
//         hub1.previewRemoveByShares(underlying, params.spoke1AddedShares),
//         'spoke1 addedAmount after'
//       );
//       assertEq(
//         hub1.getSpokeAddedShares(underlying, address(spoke1)),
...
//       // spoke2
//       assertEq(
//         hub1.getSpokeAddedAssets(underlying, address(spoke2)),
//         hub1.previewRemoveByShares(underlying, params.spoke2AddedShares),
//         'spoke2 addedAmount after'
//       );
//       assertEq(
//         hub1.getSpokeAddedShares(underlying, address(spoke2)),
