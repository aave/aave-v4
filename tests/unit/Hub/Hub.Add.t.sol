...
contract HubAddTest is HubBase {
  ...
  function setUp() public override {
    ...
    IERC20 minAllowedDecimalsErc20 = new MockERC20Decimals(
    ...
    minDecimalAssetId = hub1.addAsset(
      address(minAllowedDecimalsErc20),
      Constants.MIN_ALLOWED_UNDERLYING_DECIMALS,
  ...
  function test_add_AddCapReachedButNotExceeded_rounding() public {
    ...
    address minDecimalsUnderlying = hub1.getAsset(minDecimalAssetId).underlying;
    deal(minDecimalsUnderlying, alice, addedAmount);
    vm.prank(alice);
    IERC20(minDecimalsUnderlying).approve(address(hub1), addedAmount);
