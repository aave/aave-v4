...
contract HubBase is Base {
  ...
  function _updateAddCap(address underlying, address spoke, uint40 newAddCap) internal {
    IHub.SpokeConfig memory spokeConfig = hub1.getSpokeConfig(underlying, spoke);
    spokeConfig.addCap = newAddCap;
    vm.prank(HUB_ADMIN);
    hub1.updateSpokeConfig(underlying, spoke, spokeConfig);
  }
  ...
  function _drawLiquidity(
    ...
    Utils.draw(hub1, underlying, tempSpoke, tempUser, amount);

    if (withPremium) {
      // inflate premium data to create premium debt
      vm.prank(tempSpoke);
      hub1.refreshPremium(underlying, IHubBase.PremiumDelta(sharesDelta, premiumOffsetDelta, 0));
    }

    if (skipTime) skip(365 days);

    (uint256 drawn, uint256 premium) = hub1.getAssetOwed(underlying);
  ...
  function _drawLiquidity(address underlying, uint256 amount, bool premium) internal {
    _drawLiquidity(underlying, amount, premium, true);
  }
  ...
  function _drawLiquidityFromSpoke(
    ...
    assertTrue(hub1.getSpoke(underlying, spoke).active);

    deal(hub1.getAsset(underlying).underlying, alice, amount * 2);
  ...
  function _addLiquidity(address underlying, uint256 amount) public {
    address tempSpoke = vm.randomAddress();
    address tempUser = vm.randomAddress();

    uint256 initialLiq = hub1.getAssetLiquidity(underlying);

    address underlying = hub1.getAsset(underlying).underlying;
  ...
  function _randomUnderlying(IHub hub) internal returns (uint256) {
    return vm.randomUint(0, hub.getAssetCount() - 1);
  }

  function _randomInvalidUnderlying(IHub hub) internal returns (uint256) {
