...
// TODO:
// - test emit on accrue fees, accrual
// - test payFee to 0 address, should succeed due to no checks
// - test add reserve config with non zero LPF, zero fee receiver
...
contract LiquidityHubPayFeeWithExistingLiquidityTest is LiquidityHubBase {
  ...
  //   hub.payFeeWithExistingLiquidity(daiAssetId, amount, makeAddr('randomUser'));
  // }
  ...
  //   vm.prank(address(spoke1));
  //   hub.payFeeWithExistingLiquidity(daiAssetId, amount, alice);
  // }
  ...
  //   hub.payFeeWithExistingLiquidity(daiAssetId, 1, alice);
  // }
  ...
  //   vm.expectRevert(abi.encodeWithSelector(ILiquidityHub.SupplyCapExceeded.selector, newSupplyCap));
  //   vm.prank(address(spoke2));
  //   hub.payFeeWithExistingLiquidity(daiAssetId, 1, alice); // cannot supply any additional amount
  // }
  ...
  //   emit ILiquidityHub.AccrueFees(daiAssetId, address(spoke1), amount, amount);
  //   vm.prank(address(spoke1));
  //   hub.payFeeWithExistingLiquidity(daiAssetId, amount, alice);
  //   // hub
  ...
  //   vm.expectEmit(address(hub));
  //   emit ILiquidityHub.AccrueFees(assetId, address(spoke1), amount, amount);
  //   vm.prank(address(spoke1));
  //   hub.payFeeWithExistingLiquidity(assetId, amount, user);
  //   // hub
  ...
  //   emit ILiquidityHub.AccrueFees(assetId, address(spoke1), amount, amount);
  //   vm.prank(address(spoke1));
  //   hub.payFeeWithExistingLiquidity(assetId, amount, alice);
  //   vm.expectEmit(address(asset2));
  //   emit IERC20.Transfer(alice, address(hub), amount2);
  //   vm.expectEmit(address(hub));
  //   emit ILiquidityHub.AccrueFees(assetId2, address(spoke2), amount2, amount2);
  //   vm.prank(address(spoke2));
  //   hub.payFeeWithExistingLiquidity(assetId2, amount2, alice);
  ...
  //   vm.expectRevert(ILiquidityHub.InvalidSharesAmount.selector);
  //   vm.prank(address(spoke1));
  //   hub.payFeeWithExistingLiquidity(daiAssetId, supplyAmount, alice);
  // }
  ...
  //   vm.prank(address(spoke1));
  //   hub.payFeeWithExistingLiquidity(daiAssetId, 100e18, address(hub));
  // }
  ...
  //   vm.stopPrank();
  //   vm.prank(address(spoke2));
  //   hub.payFeeWithExistingLiquidity(daiAssetId, supplyAmount, bob);
  //   assertEq(
  //     hub.getSpokeSuppliedAmount(daiAssetId, _getFeeReceiver(daiAssetId)),
