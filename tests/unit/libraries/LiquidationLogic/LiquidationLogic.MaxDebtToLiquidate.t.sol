...
contract LiquidationLogicMaxDebtToLiquidateTest is LiquidationLogicBaseTest {
  ...
  /// function never reverts if 1 wei of debt is worth more than DUST_LIQUIDATION_THRESHOLD
  ...
  function test_calculateMaxDebtToLiquidate_fuzz_AmountAdjustedDueToDust(
    ...
    params.debtAssetPrice = bound(
      params.debtAssetPrice,
      1,
      LiquidationLogic.DUST_LIQUIDATION_THRESHOLD.fromWadDown() * params.debtAssetUnit - 1
    );
    uint256 debtToTarget = liquidationLogicWrapper.calculateDebtToTargetHealthFactor(
      _getDebtToTargetHealthFactorParams(params)
    );
  ...
  /// function reverts with MustNotLeaveDust if remaining debt is less than DUST_LIQUIDATION_THRESHOLD and debtToCover is not enough to cover all debt
