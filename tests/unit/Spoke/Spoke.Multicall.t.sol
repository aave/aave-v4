...
contract SpokeMulticall is SpokeBase {
  ...
  function test_multicall_addMultipleReserves() public {
    ...
    // Set up the multicall
    bytes[] memory calls = new bytes[](2);
    calls[0] = abi.encodeCall(ISpoke.addReserve, (daiAssetId, address(hub), _getMockOracleConfigData(spoke1, 1e8), dai2Config, dai2DynConfig));
    calls[1] = abi.encodeCall(ISpoke.addReserve, (daiAssetId, address(hub), _getMockOracleConfigData(spoke1, 1e8), dai3Config, dai3DynConfig));
