...
contract SpokeAccrueInterestTest is SpokeBase {
  ...
  function _bound(Rates memory rates) internal view returns (Rates memory) {
    rates.daiBaseBorrowRate = _bpsToRay(
      bound(rates.daiBaseBorrowRate, 1, Constants.MAX_ALLOWED_BORROW_RATE)
    ).toUint96();
    rates.wethBaseBorrowRate = _bpsToRay(
      bound(rates.wethBaseBorrowRate, 1, Constants.MAX_ALLOWED_BORROW_RATE)
    ).toUint96();
    rates.usdxBaseBorrowRate = _bpsToRay(
      bound(rates.usdxBaseBorrowRate, 1, Constants.MAX_ALLOWED_BORROW_RATE)
    ).toUint96();
    rates.wbtcBaseBorrowRate = _bpsToRay(
      bound(rates.wbtcBaseBorrowRate, 1, Constants.MAX_ALLOWED_BORROW_RATE)
    ).toUint96();
