...
contract SpokeBase is Base {
  ...
  function _calcMinimumCollAmount(
    ...
    return
      (normalizedDebtAmount.wadify() /
        normalizedCollPrice.wadify().percentMul(collData.config.liquidationThreshold)) + 1;
  }
  ...
  function _calcMaxDebtAmount(
    ...
    uint256 maxDebt = (
      (normalizedCollPrice.wadify().percentMul(collData.config.liquidationThreshold) /
        normalizedDebtAmount.wadify())
    );
