...
//     stage = 1;
//     state.withdrawnShares = hub1.previewRemoveByAssets(
//       address(tokenList.dai),
...
//     (address underlying, IERC20 underlying) = getAssetByReserveId(spoke1, params.reserveId);
...
//     assertEq(
//       hub1.getAsset(address(tokenList.wbtc)).realizedFees,
//       state.expectedFeeAmount,
//       'realized fees'
//     );
...
//     bobData[stage] = loadUserInfo(spoke1, state.reserveId, bob);
//     tokenData[stage] = getTokenBalances(underlying, address(spoke1));
//     state.withdrawnShares = hub1.previewRemoveByAssets(underlying, state.withdrawAmount);
//     uint256 addExRateBefore = getAddExRate(underlying);
...
//     assertEq(
//       tokenData[stage].hubBalance,
//       _calculateBurntInterest(hub1, underlying) + hub1.getAsset(underlying).realizedFees,
//       'tokenData hub balance'
//     );
...
//     // Check supply rate monotonically increasing after withdraw
//     uint256 addExRateAfter = getAddExRate(underlying); // caching to avoid stack too deep
...
//     state.expectedFeeAmount = _calcUnrealizedFees(hub1, underlying);
...
//     assertEq(hub1.getAsset(underlying).realizedFees, state.expectedFeeAmount, 'realized fees');
...
//     uint256 supplyExchangeRatio = hub1.previewRemoveByShares(
//       address(tokenList.dai),
//       MAX_SUPPLY_AMOUNT
//     );
//     uint256 debtExchangeRatio = hub1.previewRestoreByShares(
//       address(tokenList.dai),
//       MAX_SUPPLY_AMOUNT
//     );
...
//     assertGe(
//       hub1.previewRemoveByShares(address(tokenList.dai), MAX_SUPPLY_AMOUNT),
//       supplyExchangeRatio
//     );
//     assertGe(
//       hub1.previewRestoreByShares(address(tokenList.dai), MAX_SUPPLY_AMOUNT),
//       debtExchangeRatio
//     );
