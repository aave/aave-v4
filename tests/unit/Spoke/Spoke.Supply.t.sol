...
contract SpokeSupplyTest is BaseTest {
  ...
  // increase share conversion index on reserveId
  // supplies/draws reserve asset
  // using weth as the collateral
  ...
  function _getTokenData(address token, address user) internal returns (TokenData memory) {
