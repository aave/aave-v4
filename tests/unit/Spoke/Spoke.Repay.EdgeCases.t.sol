...
contract SpokeRepayEdgeCaseTest is SpokeBase {
  ...
  /*function test_fuzz_repay_effect_on_ex_rates(uint256 daiBorrowAmount, uint256 skipTime) public {
    ...
    uint256 debtExRateBefore = getDebtExRate(address(tokenList.dai));
    ...
    _checkDebtRateConstant(
      debtExRateBefore,
      getDebtExRate(address(tokenList.dai)),
      'after partial premium debt repay'
    );
    ...
    debtExRateBefore = getDebtExRate(address(tokenList.dai));
    ...
    _checkDebtRateConstant(debtExRateBefore, getDebtExRate(address(tokenList.dai)), 'after full debt repay');
  }
  ...
  function test_repay_supply_ex_rate_decr() public {
    ...
    // inflated to 1.5
    uint256 addExRateBefore = getAddExRate(address(tokenList.dai));
    uint256 exchangeRateBefore = hub1.previewRemoveByShares(address(tokenList.dai), MAX_SUPPLY_AMOUNT);
    ...
    _checkSupplyRateIncreasing(addExRateBefore, getAddExRate(address(tokenList.dai)), 'after borrows');
    addExRateBefore = getAddExRate(address(tokenList.dai));

    // alice repays full
    Utils.repay(spoke1, _daiReserveId(spoke1), alice, UINT256_MAX, alice);

    _checkSupplyRateIncreasing(addExRateBefore, getAddExRate(address(tokenList.dai)), 'after alice full repay');
  }
  ...
  function test_repay_supply_ex_rate_decr_skip_time() public {
    ...
    uint256 exchangeRateAfter = hub1.previewRemoveByShares(address(tokenList.dai), MAX_SUPPLY_AMOUNT);
    ...
    exchangeRateAfter = hub1.previewRemoveByShares(address(tokenList.dai), MAX_SUPPLY_AMOUNT);
  ...
  function test_repay_less_than_share() public {
    ...
    assertEq(
      ...
      hub1.previewAddByAssets(address(tokenList.weth), wethSupplyAmount)
    );
    ...
    assertEq(hub1.previewRestoreByAssets(address(tokenList.dai), repayAmount), 0, 'Shares nonzero');
  ...
  function test_repay_only_base_debt_interest() public {
    ...
    TestReturnValues memory returnValues;
    {
      ...
      emit ISpokeBase.Repay(
        ...
        bob,
        bob,
        hub1.previewRestoreByAssets(address(tokenList.dai), baseRestored),
        daiRepayAmount,
        expectedPremiumDelta
      );
    }
    ...
    assertEq(returnValues.amount, daiRepayAmount);
    assertEq(returnValues.shares, hub1.previewRestoreByAssets(address(tokenList.dai), baseRestored));
  ...
  function test_repay_only_base_debt_no_premium() public {
    ...
    emit ISpokeBase.Repay(
      ...
      bob,
      bob,
      hub1.previewRestoreByAssets(address(tokenList.dai), daiRepayAmount),
      daiRepayAmount,
      expectedPremiumDelta
    );
    vm.prank(bob);
    ...
    assertEq(returnValues.amount, daiRepayAmount);
    assertEq(returnValues.shares, hub1.previewRestoreByAssets(address(tokenList.dai), daiRepayAmount));
