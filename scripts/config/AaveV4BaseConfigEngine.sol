...
/// @title AaveV4BaseConfigEngine
/// @author Aave Labs
/// @notice Deploys and locates the `AaveV4ConfigEngine` for the Base market.
/// @dev The engine is what governance payloads delegatecall into to maintain the market after
/// launch — including the first payload, which unhalts the market. It is stateless and holds no
/// permissions, so one instance serves every future payload and it is deployed on its own rather
/// than by the deploy orchestration.
///
...
library AaveV4BaseConfigEngine {
  /// @dev Fixed salt for the Base config engine. Bump the suffix if the engine is ever redeployed,
  /// since `Create2Utils` refuses to deploy twice to the same address.
  bytes32 internal constant SALT = keccak256('AAVE_V4_BASE_CONFIG_ENGINE_V1');
