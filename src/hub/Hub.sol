...
contract Hub is IHub, AccessManaged {
  ...
  function add(uint256 assetId, uint256 amount, address from) external returns (uint256) {
    ...
    uint120 liquidity = asset.liquidity;
    uint120 assetAddedShares = asset.addedShares;
    uint120 spokeAddedShares = spoke.addedShares;
  ...
  function draw(uint256 assetId, uint256 amount, address to) external returns (uint256) {
    ...
    uint120 assetDrawnShares = asset.drawnShares;
    uint120 spokeDrawnShares = spoke.drawnShares;
  ...
  function restore(
    ...
    uint120 liquidity = asset.liquidity;
    asset.liquidity = liquidity + totalAmount.toUint120();
  ...
  function eliminateDeficit(
    ...
    uint120 deficit = coveredSpoke.deficit;
