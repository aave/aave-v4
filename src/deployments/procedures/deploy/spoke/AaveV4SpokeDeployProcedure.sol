...
contract AaveV4SpokeDeployProcedure is AaveV4DeployProcedureBase {
  ...
  /// @param gate The immutable gate used by the Spoke instance.
  ...
  function _deployUpgradeableSpokeInstance(
    ...
    require(gate != address(0), 'invalid gate');
    require(maxUserReservesLimit > 0, 'invalid max user reserves limit');
    spokeImplementation = Create2Utils.create2Deploy({
      salt: salt,
      bytecode: _getSpokeInstanceInitCode(spokeBytecode, oracle, maxUserReservesLimit, gate)
    });
  ...
  /// @param gate The gate address to encode as a constructor argument.
  ...
  function _getSpokeInstanceInitCode(
    ...
    return abi.encodePacked(spokeBytecode, abi.encode(oracle, maxUserReservesLimit, gate));
  }
}
