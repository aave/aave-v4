...
library LiquidationLogic {
  ...
  function _liquidateCollateral(
    ...
    address underlying = collateralReserve.underlying;

    uint256 sharesToLiquidate = hub.previewRemoveByAssets(underlying, params.collateralToLiquidate);
    ...
    if (params.receiveShares) {
      sharesToLiquidator = hub.previewRemoveByAssets(underlying, params.collateralToLiquidator);
      positions[params.liquidator][params.collateralReserveId].suppliedShares += sharesToLiquidator
        .toUint120();
    } else {
      sharesToLiquidator = hub.remove(underlying, params.collateralToLiquidator, params.liquidator);
    }

    if (sharesToLiquidate > sharesToLiquidator) {
      hub.payFeeShares(underlying, sharesToLiquidate.uncheckedSub(sharesToLiquidator));
    }
