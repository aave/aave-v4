...
from position_manager.interfaces import IConfigPositionManager
from position_manager.interfaces import IPositionManager

implements: IConfigPositionManager
...
def _update_permissions(spoke: address, delegator: address, delegatee: address, old_permissions: uint8, new_permissions: uint8):
    ...
    log IConfigPositionManager.UpdateConfigPermissions(
        spoke=spoke,
        delegator=delegator,
        delegatee=delegatee,
...
@internal
def _verify_permission(params: IConfigPositionManager.Permit, signature: Bytes[INF], typehash: bytes32):
...
@external
def setGlobalPermissionWithSig(params: IConfigPositionManager.Permit, signature: Bytes[INF]):
...
@external
def setCanSetUsingAsCollateralPermissionWithSig(params: IConfigPositionManager.Permit, signature: Bytes[INF]):
    PositionManagerBase._check_registered(params.spoke)
...
@external
def setCanUpdateUserRiskPremiumPermissionWithSig(params: IConfigPositionManager.Permit, signature: Bytes[INF]):
    PositionManagerBase._check_registered(params.spoke)
...
@external
def setCanUpdateUserDynamicConfigPermissionWithSig(params: IConfigPositionManager.Permit, signature: Bytes[INF]):
    PositionManagerBase._check_registered(params.spoke)
...
def setUsingAsCollateralOnBehalfOf(spoke: address, reserveId: uint256, usingAsCollateral: bool, onBehalfOf: address):
    ...
    if not ConfigPermissionsMap.can_set_using_as_collateral(permissions):
        raise IConfigPositionManager.DelegateeNotAllowed()
    ...
    log IConfigPositionManager.SetUsingAsCollateralOnBehalfOf(
        spoke=spoke,
        caller=msg.sender,
...
def updateUserRiskPremiumOnBehalfOf(spoke: address, onBehalfOf: address):
    ...
    log IConfigPositionManager.UpdateUserRiskPremiumOnBehalfOf(spoke=spoke, caller=msg.sender, onBehalfOf=onBehalfOf)
...
def updateUserDynamicConfigOnBehalfOf(spoke: address, onBehalfOf: address):
    ...
    log IConfigPositionManager.UpdateUserDynamicConfigOnBehalfOf(spoke=spoke, caller=msg.sender, onBehalfOf=onBehalfOf)
